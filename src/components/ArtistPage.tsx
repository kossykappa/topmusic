...
export default function ArtistPage({ artistId, onNavigate }: ArtistPageProps) {
  ...
  const fallbackAvatar =
    typeof artistId === 'string' ? null : artistId.live?.cover_url || null;
  ...
  async function handleBuyLicense(track: Track) {
    ...
    alert(t('licenseBought', { price: license.price }));
  }

  const artistName = artist?.name || t('artist');
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-black">
        <div className="text-xl text-white">{t('loadingArtist')}</div>
      </div>
    );
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-black">
        <div className="text-center">
          <h2 className="text-2xl text-white">{t('artistNotFound')}</h2>
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm text-gray-300">
                <span className="rounded-full bg-white/10 px-4 py-2">
                  {tracks.length} {t('releases')}
                </span>

                <span className="rounded-full bg-white/10 px-4 py-2">
                  {videoTracks.length} {t('videosLower')}
                </span>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                >
                  <span>
                    💬 {t('messageArtist')} — {artist.chat_price || 1}{' '}
                    {t('coinPerMessage')}
                  </span>
                </button>
              </div>
      ...
      <div className="mx-auto max-w-7xl space-y-14 px-6 py-12">
        <section>
          <div className="mb-6 flex items-center gap-3">
            ...
            <h2 className="text-3xl font-black text-white">{t('videos')}</h2>
          </div>
        ...
        <section>
          <div className="mb-6 flex items-center gap-3">
            ...
            <h2 className="text-3xl font-black text-white">{t('songs')}</h2>
          </div>
