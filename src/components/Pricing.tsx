...
export default function Pricing() {
  ...
  async function handleCheckout(
    ...
    if (planId === 'free') {
      alert(t('pricing.freeSelected'));
      return;
    }

    if (planId === 'premium') {
      alert(t('pricing.premiumComingSoon'));
      return;
    }
        ...
        <div className="mb-14 grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm">
            ...
            <h3 className="font-bold text-white">
              {t(
                'pricing.visibilityTitle'
              )}
            </h3>

            <p className="mt-2 text-sm text-gray-400">
              {t(
                'pricing.visibilityDescription'
              )}
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm">
            ...
            <h3 className="font-bold text-white">
              {t(
                'pricing.monetizationTitle'
              )}
            </h3>

            <p className="mt-2 text-sm text-gray-400">
              {t(
                'pricing.monetizationDescription'
              )}
            </p>
          </div>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative rounded-3xl border bg-gradient-to-br from-gray-800/50 to-gray-900/50 p-8 backdrop-blur-sm ${
                ...
                plan.popular
                  ? 'scale-105 border-2 shadow-2xl shadow-red-500/20'
                  : ''
              } ${
                plan.disabled
                  ? 'opacity-85'
